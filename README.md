...
`tools/list` at `https://gateway.pipeworx.io/data-sncf/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/data_sncf_search_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"horaires"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/data_sncf_search_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
